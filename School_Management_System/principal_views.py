...
def principal_home(request):
    # they will return total no of teachre, studnt ,courses
...
def add_student(request):
    ...
    if request.method == "POST":
        ...
        x = 0
        d = validations(profile_pic, first_name, last_name, email, username)
        for i in d.values():
        ...
        #     return redirect('add_student')
            ...
            messages.success(request, user.first_name + "  " +
                             user.last_name + " student are Successfully Added !")
            return redirect('add_student')
...
def update_course(request):
    if request.method == "POST":
        # form ky ander course name ko bhaja tha name ky title sy osko yahan per get kiya ha
        name = request.POST.get('name')
        # or course id bhaji thi edit_course ma wo get krwai
        course_id = request.POST.get('course_id')

        # models ma say course object ki id mil jay gi issy
        course = Course.objects.get(id=course_id)
...
def add_teacher(request):
    if request.method == "POST":     # it will get the post request
        # this will get the data from fom and save it
        profile_pic = request.FILES.get('profile_pic')
        ...
        if Customuser.objects.filter(username=username).exists():
            ...
            # password is equal to password nahi likh sakty is liya
            user.set_password(password)
            user.save()
